...
import { X, Send, Clock, CheckCircle, XCircle, Users, ChevronDown, ChevronUp, Star, Shield, AlertCircle, UserCheck, RefreshCw } from 'lucide-react';
import { supabase } from '../../supabaseClient';
import { getPositionLabel, positionMatches } from '../../utils/positionHelpers';

export default function InviteWorkersModal({
  open,
  event,
  workers,
  assignments,
  ...
  // Workers already assigned to this event
  ...
  // Invitations for selected position
  ...
  // Already invited worker IDs for this position
  const invitedWorkerIds = new Set(
    positionInvites
      .filter(i => ['pending', 'accepted', 'confirmed', 'standby'].includes(i.status))
      .map(i => i.worker_id)
  );

  // Eligible workers by rank for selected position
  ...
  const selectAllRank = (rank) => {
    ...
    setSelectedWorkers(prev => {
      ...
      if (allSelected) {
        rankWorkers.forEach(w => next.delete(w.id));
      } else {
        rankWorkers.forEach(w => next.add(w.id));
      }
      return next;
    });
  };
  ...
  const handleConfirmWorker = async (invitation) => {
    ...
    try {
      // Confirm this worker → becomes a real assignment
  ...
  const handleExpireAndCascade = async (rank) => {
    // Mark pending invites for this rank as expired
  ...
  const getWorkerName = (workerId) => workers.find(w => w.id === workerId)?.name || 'Unknown';
  const getWorkerRank = (workerId) => workers.find(w => w.id === workerId)?.rank || 1;
  const getWorkerReliability = (workerId) => workers.find(w => w.id === workerId)?.reliability ?? 5.0;
  ...
  // Summary counts for responses tab badge
  const totalAccepted = invitations.filter(i => i.position === selectedPosition && i.status === 'accepted').length;
  const totalPending = invitations.filter(i => i.position === selectedPosition && i.status === 'pending').length;
              ...
              <div className="flex items-center space-x-4 text-sm">
                ...
                {acceptedInvites.length > 0 && (
                  <span className="flex items-center space-x-1 text-green-700 font-medium">
                    <CheckCircle size={14} />
                    <span>{acceptedInvites.length} accepted</span>
                  </span>
                )}
            ...
            <div className="px-6 pt-3 flex space-x-1 flex-shrink-0">
              <button
                ...
                className={`px-4 py-2 rounded-t-lg text-sm font-medium transition-colors ${
                  activeTab === 'invite' ? 'bg-white border border-b-white border-gray-200 text-red-900 -mb-px z-10' : 'text-gray-500 hover:text-gray-700'
                }`}
              >
                Send Invites
              </button>
              <button
                ...
                className={`px-4 py-2 rounded-t-lg text-sm font-medium transition-colors flex items-center space-x-1.5 ${
                  activeTab === 'responses' ? 'bg-white border border-b-white border-gray-200 text-red-900 -mb-px z-10' : 'text-gray-500 hover:text-gray-700'
                }`}
              >
                <span>Responses</span>
                {(totalAccepted + totalPending) > 0 && (
                  <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${
                    totalAccepted > 0 ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                  }`}>
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    {[1,2,3,4,5].map(r => (
                      ...
                      >
                        R{r}
                        {workersByRank[r]?.length > 0 && (
                          <span className="ml-1 text-xs opacity-75">({workersByRank[r].length})</span>
                        )}
                      </button>
                    ))}
                  </div>

                  {/* Workers for selected rank */}
                  {[5,4,3,2,1].filter(r => r === selectedRank).map(rank => {
                    const rankWorkers = workersByRank[rank] || [];
                      ...
                      <div key={rank} className="border rounded-lg overflow-hidden">
                        ...
                        {rankWorkers.length === 0 ? (
                          <div className="px-4 py-6 text-center text-gray-400 text-sm">
                            No eligible Rank {rank} workers for this position
                          </div>
                        ) : (
                          <div className="divide-y">
                            {rankWorkers.map(worker => {
                              ...
                              const reliabilityColor = reliability >= 4.5 ? 'text-green-700' :
                                reliability >= 3.5 ? 'text-yellow-600' : 'text-red-600';
                              ...
                              return (
                                ...
                                >
                                  <div className={`w-5 h-5 rounded border-2 flex-shrink-0 flex items-center justify-center transition-colors ${
                                    isSelected ? 'bg-red-900 border-red-900' : 'border-gray-300'
                                  }`}>
                                  ...
                                  <div className="flex items-center space-x-3 text-xs flex-shrink-0">
                                    <span className={`flex items-center space-x-0.5 font-medium ${reliabilityColor}`}>
                    ...
                    <div>
                          ...
                          <div key={inv.id} className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-lg">
                            <div>
                              ...
                              <p className="text-xs text-gray-500">
                                Rank {getWorkerRank(inv.worker_id)} · ⭐ {getWorkerReliability(inv.worker_id).toFixed(1)} · Accepted {inv.responded_at ? new Date(inv.responded_at).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }) : ''}
                              </p>
                            </div>
                            <button
                              onClick={() => handleConfirmWorker(inv)}
                              disabled={confirming === inv.id || openSlots <= 0}
                      ...
                      {openSlots <= 0 && (
                        <p className="text-xs text-orange-600 mt-1 flex items-center space-x-1">
                          <AlertCircle size={12} />
                          <span>All slots filled — confirming will move others to standby</span>
                        </p>
                      )}
                    </div>
                  )}
                  ...
                  {pendingInvites.length > 0 && (
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="text-sm font-bold text-yellow-800 flex items-center space-x-1.5">
                        ...
                        {pendingInvites.map(inv => (
                          <div key={inv.id} className="flex items-center justify-between p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                            ...
                            <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                              isExpired(inv) ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'
                            }`}>
                              {formatExpiry(inv)}
                            </span>
                          </div>
                  ...
                  {confirmedInvites.length > 0 && (
                    <div>
                      <h4 className="text-sm font-bold text-gray-700 mb-2 flex items-center space-x-1.5">
                        ...
                        <span>Confirmed ({confirmedInvites.length})</span>
                      </h4>
                      <div className="space-y-2">
                  ...
                  {standbyInvites.length > 0 && (
                    <div>
                      <h4 className="text-sm font-bold text-blue-700 mb-2 flex items-center space-x-1.5">
                        ...
                        <span>Standby ({standbyInvites.length})</span>
                      </h4>
                      <div className="space-y-2">
                        {standbyInvites.map(inv => (
                          <div key={inv.id} className="flex items-center justify-between p-3 bg-blue-50 border border-blue-200 rounded-lg">
                            <div>
                              ...
                              <p className="text-xs text-gray-500">Was accepted — moved to standby when slot filled</p>
                            </div>
                            <button
                              onClick={() => handleConfirmWorker(inv)}
