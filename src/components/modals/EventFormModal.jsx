...
// around this) -- create vs. edit mode is determined entirely by whether
// `event` is null. The two modals used to be ~90% duplicated copies; every
// mode-specific difference below is intentional and commented, not an
// oversight.
export default function EventFormModal({
  ...
  // Load active locations/venues/clients fresh every time the modal opens.
  // Both modals used to do this, but EditEventModal's version had no `open`
  // dependency and fetched only once, ever, on the app's initial mount
  // (both modals are rendered unconditionally in App.jsx, just toggled via
  // `open`, so it never re-ran) -- a venue or client added mid-session
  // would show up when creating a new event but not when editing one,
  // until a full page reload. Standardized on the create modal's
  // refetch-on-open behavior for both, since it's strictly fresher data at
  // the cost of one extra read per open.
  useEffect(() => {
    if (!open) return;
    supabase
      .from('locations')
      ...
      .then(({ data }) => {
        setLocations(data || []);
        // Auto-select the only market when creating a new event. Not done
        // in edit mode -- location_id there always comes from the event
        // itself (including a genuine null), and auto-selecting could
        // silently overwrite that.
        if (!isEdit && data && data.length === 1) {
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      flat_pay_amount: event.flat_pay_amount != null ? String(event.flat_pay_amount) : '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // window is already open the moment this event exists, instead of
      // waiting for the next hourly cron tick. The cron's normal mode only
      // fires on the exact day a rank's window opens, so a rank whose
      // window opened before this event was even created would otherwise
      // never be notified.
      if (insertedId) {
        fetch('/api/send-availability-notifications', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ catchup: true, event_id: insertedId })
        }).catch(() => {});
      }
      ...
      notify((isEdit ? ERROR_MESSAGES.DATA.UPDATE_FAILED : ERROR_MESSAGES.DATA.SAVE_FAILED) + ': ' + error.message);
    } finally {
      setSaving(false);
    }
  };
  ...
  const closeAndReset = () => {
    setFormData({
      ...
      client_id: null, invite_only: false, staffing_mode: 'approval', flat_pay_amount: '',
      meeting_point_description: '', meeting_point_url: '', meeting_point_lat: null, meeting_point_lng: null
    });
        ...
        <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full">
          <div className="p-6 max-h-[85vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-2xl font-bold text-gray-900">{isEdit ? 'Edit Event' : 'Create New Event'}</h3>
              <button
                onClick={handleClose}
